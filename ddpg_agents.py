...
class DDPGAgents:
    ...
    def act(self, states: torch.Tensor, noise_scale: float) -> List[np.ndarray]:
        """
        Get actions from all agents

        Args:
            states: state for each agent
            noise_scale: the amount of noise to add to action values
        """
        actions = []
        for i, ddpg_agent in enumerate(self.ddpg_agents):
            state = states[i]
            noise = np.random.normal(scale=noise_scale, size=ddpg_agent.action_size)
            action = convert_to_numpy(ddpg_agent.act(state[None, :])) + noise
